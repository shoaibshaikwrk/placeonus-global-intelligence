...
def gdelt_news():
    ...
    for a in raw.get('articles', []):
        url = a.get('url'); title = (a.get('title') or '').strip()
        if not url or not title or url in seen:
            continue
        seen.add(url)
        items.append({'headline': title,'source': a.get('domain') or 'GDELT source','region': a.get('sourcecountry') or 'Global','language': a.get('language'),'published_at': a.get('seendate'),'url': url,'image': a.get('socialimage'),'category': 'GLOBAL INTELLIGENCE','summary': 'Current article discovered through GDELT. Open the source for full reporting.','live': True})
...
def latest_eia_value(raw):
    try:
        ...
        if not rows:
            return None
        row = rows[0]; value = row.get('value')
...
def market_data():
    ...
    try:
        ...
        if wti: instruments.append({'name':'WTI','price':wti['price'],'trend':'official EIA spot','signal_score':None,'period':wti['period'],'live':True})
        if brent: instruments.append({'name':'Brent','price':brent['price'],'trend':'official EIA spot','signal_score':None,'period':brent['period'],'live':True})
        if wti or brent: source.append('EIA')
...
# Target risk corridors plus a few high-traffic validation areas so a healthy
# AISStream connection is visibly testable on every scheduled sample.
AISSTREAM_BOXES = [
    [[22.0, 54.0], [28.5, 60.5]],       # Persian Gulf / Strait of Hormuz
    [[11.0, 41.0], [30.5, 45.5]],       # Red Sea / Bab el-Mandeb
    [[29.0, 31.0], [31.5, 33.5]],       # Suez Canal
    [[40.0, 27.0], [47.5, 42.5]],       # Black Sea
    [[0.0, 98.0], [6.5, 106.0]],        # Malacca / Singapore
    [[7.0, -83.0], [10.5, -77.0]],      # Panama Canal
    [[50.5, 2.5], [53.0, 6.0]],         # Rotterdam / North Sea validation
    [[25.3, -80.7], [26.1, -79.6]],     # Miami validation
]
    ...
    try:
        async with websockets.connect('wss://stream.aisstream.io/v0/stream', open_timeout=10, close_timeout=5, ping_interval=20, ping_timeout=20, compression='deflate') as ws:
            ...
            while time.monotonic() - started < duration_seconds and len(vessels) < max_vessels:
                ...
                mmsi = meta.get('MMSI'); lat = meta.get('Latitude'); lon = meta.get('Longitude')
...
def derive_risk(news):
    ...
    return {'source':'PlaceOnUs derived from current headlines','live':True,'scores':{'geopolitical':geo,'oil_supply':oil,'shipping':shipping,'freight':freight,'marine_insurance':insurance},'method':'Transparent keyword-pressure heuristic; AI model not yet enabled.'}
...
def main():
    ...
    vessels = vessel_data(); write('vessels.json',vessels); status['sources']['vessels']='ok' if vessels.get('live') else 'connected_no_positions' if vessels.get('subscription_confirmed') else 'error'
    ...
    status['refresh_minutes']=10; status['site']='news.placeonus.com'; write('system-status.json',status)
